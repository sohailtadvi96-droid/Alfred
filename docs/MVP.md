...
5. Office work (Work module, second tab) — migrations 0009 (`office_tasks`, `office_events`, `office_notes`), 0010 (`office_notes.entry_date`, `office_journal`)
   ...
   - ✅ Per-day page `/work/day/:date` — Schedule (local meetings + that day's Google events, add/edit local), Due (tasks with that due date, add pre-dated; the *today* page also rolls up still-open past-due tasks under an Overdue divider), Journal (one free-text entry per day, autosaves on blur), Notes (short notes filed to that date). Prev/next-day nav.
   - ✅ **Tasks backlog** on the landing — every open task grouped Overdue / No date / Upcoming (with a due chip), inline add, collapsible done list. Cell hover shows a preview of that day's meetings, tasks, note count and journal first line.
   - ✅ Polish: persistent **Today** jump button on the calendar; notes (running + dated) are click-to-edit in place; a **Journal** archive panel on the landing lists recent entries → day page; tighter calendar grid on ≤560px.
   - ✅ Optional Google Calendar read-only sync via GIS token client (`VITE_GOOGLE_CLIENT_ID`); events merged into calendar dots + the day schedule, never stored. Degrades to a setup note when unconfigured. Fetches per on-screen window (the visible month grid, or a single day) rather than a fixed horizon; connection state is a shared module store so the calendar and every day page use one token.
