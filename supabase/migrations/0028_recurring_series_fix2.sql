-- 0028_recurring_series_fix2.sql
-- 0027 fixed the uuid aggregate but missed that `current_date - (v_interval
-- * 1.5)` produces numeric (1.5 is a float literal), and `date - numeric`
-- isn't a valid Postgres operator (only `date - integer`). Round it.
  ...
  loop
    ...
    if v_existing_id is not null then
      ...
      set entity_id = v_row.entity_id,
          ...
          status = case when v_row.last_seen < (current_date - round(v_interval * 1.5)::integer) then 'lapsed' else 'active' end
      ...
      insert into public.recurring_series (
        ...
        case when v_row.last_seen < (current_date - round(v_interval * 1.5)::integer) then 'lapsed' else 'active' end
      );
    end if;
    v_found := v_found + 1;
  end loop;
